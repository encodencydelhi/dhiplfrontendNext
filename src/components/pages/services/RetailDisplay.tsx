...
const RetailDisplayPage = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const { data, portfolioGalleryImages } = useServiceDetail("Retail Display Merchandising");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Retail Display Merchandising"
        fallbackImage="https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Retail Display Merchandising" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/merchandising/retail-display-images"
          />
        )}
        <Location category="Retail Display Merchandising" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
...
export default RetailDisplayPage;
