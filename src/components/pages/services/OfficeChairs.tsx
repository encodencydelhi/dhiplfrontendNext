...
const OfficeChairsPage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Chairs");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <DynamicHero 
        pageName="Chairs" 
        fallbackImage="https://images.unsplash.com/photo-1505797149-43b007664976?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Chairs" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/office-interior/office-chairs-images"
          />
        )}
        <Location category="Office Chairs" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
