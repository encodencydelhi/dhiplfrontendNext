...
const WardrobePage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Modular Wardrobe");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Modular Wardrobe"
        fallbackImage="https://images.unsplash.com/photo-1558997519-83ea9252edf8?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Modular Wardrobe" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/furniture/wardrobes-images"
          />
        )}
        <Location category="Wardrobes" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
