...
const MDCabinPage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("MD Cabin");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="MD Cabin" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/office-interior/md-cabin-images"
          />
        )}
        <Location category="MD Cabins" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
