...
const AcrylicDisplayPage = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const { data, portfolioGalleryImages } = useServiceDetail("Acrylic Displays");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Acrylic Displays"
        fallbackImage="https://images.unsplash.com/photo-1567401893414-76b7b1e5a7a5?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Acrylic Displays" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/merchandising/acrylic-images"
          />
        )}
        <Location category="Acrylic Displays" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
...
export default AcrylicDisplayPage;
