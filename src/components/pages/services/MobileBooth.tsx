...
const MobileBoothPage = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const { data, portfolioGalleryImages } = useServiceDetail("Mobile Booth");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Mobile Booth" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/kiosk/booth-images"
          />
        )}
        <Location category="Mobile Booths" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
...
export default MobileBoothPage;
