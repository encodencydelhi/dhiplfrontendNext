...
const GondolasPage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Gondolas");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Gondolas" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/merchandising/gandola-images"
          />
        )}
        <Location category="Gondolas" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
