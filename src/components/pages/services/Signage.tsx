...
const SignagePage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Signage");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Signage" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/signage/signage-portfolio"
          />
        )}
        <Location category="Signage" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
