...
const CorporateInteriorPage = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const { data, portfolioGalleryImages } = useServiceDetail("Corporate Interior");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Corporate Interior"
        fallbackImage="https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Corporate Interior" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/interior/corporate-interior"
          />
        )}
        <Location category="Corporate Interiors" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
...
export default CorporateInteriorPage;
