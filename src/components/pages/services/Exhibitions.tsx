...
const ExhibitionsPage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Exhibition & Events");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Exhibition & Events"
        fallbackImage="https://images.unsplash.com/photo-1559136555-9303baea8ebd?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Exhibition & Events" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/signage/exhibition-events-portfolio"
          />
        )}
        <Location category="Exhibition & Events" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
