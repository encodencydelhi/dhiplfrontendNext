...
const WindowDisplayPage = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const { data, portfolioGalleryImages } = useServiceDetail("Window Display");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Window Display" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/merchandising/window-display-images"
          />
        )}
        <Location category="Window Displays" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
...
export default WindowDisplayPage;
