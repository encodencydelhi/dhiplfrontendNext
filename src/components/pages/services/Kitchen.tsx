...
const KitchenPage = () => {
  ...
  const { data, portfolioGalleryImages } = useServiceDetail("Modular Kitchen");
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <Navbar />
      <DynamicHero
        pageName="Modular Kitchen"
        fallbackImage="https://images.unsplash.com/photo-1556911220-e15595b6a281?w=1920&q=80"
        overrideImage={data?.bgImage}
        overrideTitle={data?.bgTitle}
        overrideHighlight={data?.bgHighlightTitle}
      />
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-20">
        <ServiceContentSection serviceName="Modular Kitchen" />
        {portfolioGalleryImages && portfolioGalleryImages.length > 0 && (
          <RecentWorkSection
            galleryItems={portfolioGalleryImages}
            onImageClick={setSelectedImage}
            viewAllLink="/portfolio/furniture/kitchen-images"
          />
        )}
        <Location category="Kitchens" />
      </div>
      <Footer />
      <AnimatePresence>
        {selectedImage && (
